...
// als Link auf die Website, weil die aktuell live Seite noch die alte
// Version ohne diese Datei ist). Referenziert im HTML via "cid:logo".
...
serve(async (req) => {
  ...
  try {
    ...
    // Laenge deckeln — verhindert ueberdimensionierte Eingaben (DB/E-Mail).
    if (customer_name.length > 200 || customer_email.length > 200
        || (customer_phone && customer_phone.length > 60)
        || (customer_address && customer_address.length > 400)) {
      return json({ success: false, error: "INVALID_VOUCHER_CODE" }, 400);
    }
    ...
    // Reservierung läuft komplett atomar in der DB-Funktion:
    // sperrt Voucher + Slot (FOR UPDATE), prüft Status/Ablauf/Doppelbuchung/
    // Kapazität und setzt Reservierung + Slot-Kapazität + Voucher-Status
    // in EINER Transaktion. Kein manuelles Nachbauen der Logik hier nötig.
    const { data: result, error } = await supabase.rpc("reserve_voucher_slot", {
      p_voucher_code:     voucher_code.toUpperCase().trim(),
      p_slot_id:          slot_id,
    ...
    if (error) {
      console.error("schedule-voucher rpc:", error);
      return json({ success: false, error: "UNKNOWN" }, 500);
    }

    if (!result?.success) {
      return json({ success: false, error: result?.error || "UNKNOWN" });
    }

    // Bestätigungs-E-Mail verschicken — Fehler dabei dürfen die erfolgreiche
    // Buchung nicht mehr rückgängig machen, nur geloggt werden.
    try {
      const resendKey = Deno.env.get("RESEND_API_KEY");
      if (resendKey) {
        const { data: voucher } = await supabase
          .from("vouchers")
          .select("tasting_type, persons")
        ...
        await sendBookingConfirmation({
          ...
          to: customer_email,
          name: customer_name,
          tastingLabel: TASTING_LABELS[voucher?.tasting_type ?? ""] ?? "Tasting",
      ...
      console.error("schedule-voucher confirmation email:", mailErr);
    }
...
// HTML-escapen fuer Werte aus Nutzereingaben (der Name, den jemand im
// Einloese-Formular eintraegt), bevor sie in die E-Mail-Vorlage eingesetzt
// werden — verhindert HTML-/Markup-Injection in der versendeten Mail.
...
async function sendBookingConfirmation(opts: {
  ...
  const { resendKey, to, name, tastingLabel, persons, slotDate, slotTime } = opts;
  const personsLabel = `${persons} ${persons === 1 ? "Person" : "Personen"}`;
  const dateLabel = new Date(slotDate + "T00:00:00").toLocaleDateString("de-DE", {
    day: "2-digit", month: "long",
  });
  const dateLabelFull = new Date(slotDate + "T00:00:00").toLocaleDateString("de-DE", {
    weekday: "long", day: "2-digit", month: "long", year: "numeric",
  });
  const timeLabel = String(slotTime).slice(0, 5);
...
<table width="460" cellpadding="0" cellspacing="0" style="background:#fffdf9;border-radius:18px;overflow:hidden;">
  ...
  <tr><td style="background:#1c3a2e;padding:30px 20px 36px;text-align:center;">
    ...
    <p style="margin:18px 0 0;font-family:Helvetica,Arial,sans-serif;font-size:11px;letter-spacing:3px;text-transform:uppercase;color:#e7d6a4;">Terminbestätigung</p>
    <h1 style="margin:8px 0 0;font-size:24px;font-weight:400;color:#f9f5ef;">Wir freuen uns auf dich.</h1>
  </td></tr>
  ...
  <tr><td style="padding:28px 32px 6px;text-align:center;">
    <p style="margin:0;font-family:Helvetica,Arial,sans-serif;font-size:14.5px;line-height:1.75;color:#55503f;">
      Liebe/r ${esc(name)}, dein Platz ist reserviert &ndash; hier ist dein persönliches Tasting-Ticket.
    </p>
  </td></tr>

  <tr><td style="padding:14px 32px 0;text-align:center;">
    <p style="margin:0;font-size:25px;color:#1c3a2e;">${tastingLabel}</p>
    <p style="margin:4px 0 24px;font-family:Helvetica,Arial,sans-serif;font-size:12px;color:#c9a84c;letter-spacing:1.5px;text-transform:uppercase;font-weight:700;">${personsLabel}</p>
  </td></tr>

  <tr><td style="padding:0 32px 28px;">
    <table width="100%" cellpadding="0" cellspacing="0">
      <tr>
        <td width="50%" style="text-align:center;padding:0 6px;border-right:1px solid rgba(28,58,46,0.14);">
          <span style="display:block;font-family:Helvetica,Arial,sans-serif;font-size:10px;letter-spacing:1.5px;text-transform:uppercase;color:#55503f;margin-bottom:6px;">Datum</span>
          <span style="display:block;font-size:18px;color:#26241d;font-weight:bold;">${dateLabel}</span>
        </td>
        <td width="50%" style="text-align:center;padding:0 6px;">
          <span style="display:block;font-family:Helvetica,Arial,sans-serif;font-size:10px;letter-spacing:1.5px;text-transform:uppercase;color:#55503f;margin-bottom:6px;">Uhrzeit</span>
          <span style="display:block;font-size:18px;color:#26241d;font-weight:bold;">${timeLabel}</span>
        </td>
      </tr>
    </table>
  </td></tr>
  ...
  <tr><td style="background:#12241c;padding:22px 30px 26px;text-align:center;">
    ...
    <p style="margin:0 0 12px;font-family:Helvetica,Arial,sans-serif;font-size:12.5px;color:rgba(249,245,239,0.72);line-height:1.6;">
      ${dateLabelFull}, ${timeLabel} Uhr<br>
  ...
  const res = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      to: [to],
      subject: `Dein Termin am ${dateLabel} ist bestätigt`,
      html,
      attachments: [
        {
  ...
  if (!res.ok) {
    ...
    console.error("Resend error (booking confirmation):", err);
  }
}
